...
public class a_control extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(flag.equals("setclinicname")){
			System.out.println("1");
			setclinicname(request,response);
			response.sendRedirect("clinicJSON.jsp");
		}
	}

	private void setclinicname(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		...
		d_dao dd=new d_dao();
		doctor ls=dd.fetchdoctor(d);
//		System.out.println(ls.size());
		
		String cname=ls.getClinic();
		
		HttpSession h=request.getSession();
		h.setAttribute("clinic",cname);
	...
	private void updateappoinment(HttpServletRequest request, HttpServletResponse response) {
		...
		d.setId(did);
		d_dao dd=new d_dao();
		doctor doc=dd.fetchdoctor(d);
		
		String dname=doc.getFn();
